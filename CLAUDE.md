...
- The `changes` path filter means a PR touching only `.github/**`, `*.md`, `LICENSE` or `.gitignore` **skips every heavy job**, so all its required checks report as passing without having run. That is by design (a skipped workflow would never report at all and would hang forever at "Expected"), but it does mean a CI-only PR cannot self-verify — the change lands and the *next* code PR exercises it.
